    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "detached" and sys.argv[2] and sys.argv[3]:
            ...
            for n in len(db):
                ...
                imclean = "floorplan_", floor, ".png"
                heatmap(lat=db[floor[1]], long=db[floor[0]], c=db[floor[2]], cmap=pl.cm.rainbow, boundry=bbox, vmin=1,
                    vmax=m, imname=imclean, s=DotScale, alph=0.4, pname=title, time=current_time,
        ...
        if sys.argv[1] == "user":
            ...
            print("For user mode, user parameter [user]. \n"
              ...
              "[http:localhost:8008] and [familyname] so: heatmap.py live http:localhost:8008 familyname")
        else:
...
def introduction():
    ...
    if user_in == "live":
        ...
        print("Please specify the address of the FIND3 server [e.g. http://localhost:8008]")
        user_in = input()
        u = user_in
